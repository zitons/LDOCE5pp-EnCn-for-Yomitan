...
ap.add_argument('--revision',default=date.today().strftime('%Y.%m.%d')+'-review-fixes')
args=ap.parse_args()
out=(args.output or ROOT/'yomitan_fixed'/'2026-09-14-n1-n3'/args.mode).resolve()
...
cmd=[sys.executable,'-X','utf8','-u',str(source),'-i',str(ROOT/'extract/LDOCE5++ V 2-15.mdx.txt'),
     '-o',str(out),'-m',args.mode,'--revision',args.revision,'--no-progress']
